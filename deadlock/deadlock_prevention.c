        ...
        uint32_t **mat_need, size_t n, size_t m)
{
    ...
    while (arr_ne(finish, all_true, n))
    {
        ...
        for (i = 0; i < n && (finish[i] || (arr_g_any(mat_need[i], work, m))); i++);
        if (i == n)
        {
            // Free memory.
            kfree(work);
...
deadlock_status_t request(uint32_t *req_vec, size_t task_i,
        ...
        size_t n, size_t m)
{
    if (arr_g_any(req_vec, mat_need[task_i], m))
    {
        return ERROR;
    }

    if (arr_g_any(req_vec, arr_available, m))
    {
        return WAIT;
    }

    // Try to allocate resources.
    arr_available = arr_sub(arr_available, req_vec, m);
    mat_alloc[task_i] = arr_add(mat_alloc[task_i], req_vec, m);
    mat_need[task_i] = arr_sub(mat_need[task_i], req_vec, m);

    // Check safe state
    if (!state_safe(arr_available, mat_alloc, mat_need, n, m))
    {
        // Restore previous allocation.
        arr_available = arr_add(arr_available, req_vec, m);
        mat_alloc[task_i] = arr_sub(mat_alloc[task_i], req_vec, m);
        mat_need[task_i] = arr_add(mat_need[task_i], req_vec, m);
        return WAIT_UNSAFE;
    }
    return SAFE;
}
